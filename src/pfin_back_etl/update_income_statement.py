...
Description:
    Update the pfin.income_statement table
...
with sqla.orm.Session(engine) as session:
    ...
    # 1. Fetch what's already in pfin.income_statement for later comparison
    print(f"Figure out what's already in pfin.income_statement...")
    tab_sbase = Base.by_module.pfin.income_statement
    ...
    # 2. Fetch the stock income_statement data from FMP
    print(f"Fetching income_statement data from Financial Modeling Prep...")
    df_fmp = fetch_fmp_list_df(fmp_client.income_statement, 'symbol',
    ...
    # 3. Find the common columns to populate in the income_statement DB table
    print(f"Merging columns (inner join) to limit what gets sent to DB...")
    t_sbase = Base.by_module.pfin.income_statement
