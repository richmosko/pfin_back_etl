...
class SBaseConn:
    ...
    def _sbase_setup(self):
        ...
        base.prepare(autoload_with=engine, modulename_for_table=utils.sqla_modulename_for_table)
    ...
    def get_column_dict(self, tab_obj):
        ...
        for column in columns:
            ...
            print(f"  Column Name: {column.name}, Type: {column.type}[{type(column.type)}]")
        return c_dict
...
class PFinFMP(fmpstab.FMPStab):
    ...
    def __init__(self, api_key: str) -> None:
        ...
        logger = None
        log_enabled = False
        super().__init__(api_key, max_calls_per_minute, config_file, base_url, logger, log_enabled)
    ...
    def get_screened_stocks(self, min_mkt_cap, result_limit):
        ...
        """
        df_slist = self.fetch_fmp_df(self.company_screener,
...
class PFinBackend(SBaseConn):
    ...
    def update_table_cpi(self):
        ...
        (common_cols, df_old, df_new) = self._calc_common_cols_df(tab_sbase, df_sbase, df_api)
        #print(common_cols)
    ...
    def update_table_asset(self):
        ...
        sym_list = ['NVDA', 'AAPL', 'IREN', 'V', 'ALAB', 'APP', 'GOOGL', 'META', 'ABXL', 'MSFT']

        print(f"Fetching data from Financial Modeling Prep...")
        df_fmp = self.fmp_client.fetch_fmp_list_df(self.fmp_client.search_symbol,
                                                   'query',
                                                   query=sym_list,
                                                   limit=1)
        ...
        print(f"Merging columns to (inner join) to limit what gets sent to DB...")
        (common_cols, df_old, df_new) = self._calc_common_cols_df(tab_sbase, df_sbase, df_fmp)
        #print(common_cols)
    ...
    def update_table_equity_profile(self):
        ...
        print(f"Fetching data from Financial Modeling Prep...")
        key_list = ['symbol']
        df_fmp = self.fmp_client.fetch_fmp_list_df(self.fmp_client.profile,
                                                   'symbol',
                                                   symbol=sym_list,
                                                   limit=1)
    ...
    def update_table_reporting_period(self):
        ...
        print(f"Fetching data from Financial Modeling Prep...")
        df_fmp = self.fmp_client.fetch_fmp_list_df(self.fmp_client.income_statement,
                                                   'symbol',
                                                   symbol=sym_list,
        ...
        df_fmp = df_fmp.rename({'symbol': 'asset_id'})
        df_fmp = df_fmp.with_columns(
                 pl.col('asset_id').replace(asset_map).str.to_integer(strict=False)
                 .alias('asset_id')
        )
        df_fmp = df_fmp.with_columns(
                 pl.col('fiscal_year').str.to_integer(strict=False)
                 .alias('fiscal_year')
        )
        ...
        df_fmp = df_fmp.with_columns(
                 pl.col('filing_date').str.to_date(strict=False)
                 .alias('filing_date')
        )
        df_fmp = df_fmp.with_columns(
                 pl.col('accepted_date').str.to_datetime(strict=False, time_zone="UTC")
                 .alias('accepted_date')
        )

        print(f"Create generic 'future' reporting periods for EPS & Rev estimates...")
        tmp_date_now = datetime.now(timezone.utc)
        tmp_date_fut = datetime.fromisoformat(self._tmp_date_fut).replace(tzinfo=timezone.utc)
    ...
    def update_table_balance_sheet_statement(self):
        ...
        df_fmp = self.fmp_client.fetch_fmp_list_df(self.fmp_client.balance_sheet_statement,
                                                   'symbol',
                                                   symbol=sym_list,
                                                   limit=PERIODS_TO_FETCH,
                                                   period='quarter')
    ...
    def update_table_cash_flow_statement(self):
        ...
        df_fmp = self.fmp_client.fetch_fmp_list_df(self.fmp_client.cash_flow_statement,
                                                   'symbol',
                                                   symbol=sym_list,
                                                   limit=PERIODS_TO_FETCH,
                                                   period='quarter')
    ...
    def update_table_earning(self):
        ...
        df_fmp = self.fmp_client.fetch_fmp_list_df(self.fmp_client.earnings,
                                                   'symbol',
                                                   symbol=sym_list,
                                                   limit=(PERIODS_TO_FETCH+2))
        df_fmp = utils.clean_empty_str_df(df_fmp)
        df_fmp = df_fmp.filter(
                 ~(pl.col('revenue_actual').is_null() & pl.col('revenue_estimated').is_null())
        )
        df_fmp = df_fmp.rename({'symbol': 'asset_id'})
        ...
        df_fmp = df_fmp.with_columns(
                 pl.col('filing_date').alias('ref_date')
        )
        df_fmp = df_fmp.with_columns(
            pl.lit(None).alias('reporting_period_id')
        )
        #print(df_fmp)

        print(f"Match earnings reports to posted reporting_periods...")
        # sort and add a temporary row_idx as primary keys
        df_rp_map = df_rp_map.sort('filing_date', descending=True).with_row_index(name='row_idx')
        df_fmp = df_fmp.sort('filing_date', descending=True).with_row_index(name='row_idx')

        fmp_drop_list = []
        for asset_id in id_list:
            cond_fmp_asset_id = (pl.col('asset_id') == asset_id)
            cond_fmp_filing_date = (pl.col('filing_date') <= latest_rpt[asset_id]+timedelta(weeks=2))
            # filter for the conditions above, and get the row_idx values as lists
            fmp_idx = df_fmp.filter(cond_fmp_asset_id & cond_fmp_filing_date)['row_idx'].to_list()
            rpm_idx = df_rp_map.filter(cond_fmp_asset_id & cond_fmp_filing_date)["row_idx"].to_list()
            ...
            rpm_list = df_rp_map.filter(pl.col("row_idx").is_in(rpm_idx))['id'].to_list()
            df_map = pl.DataFrame({
                'row_idx': fmp_idx,
                'reporting_period_id': rpm_list
            })
            # now find the index rows in df_fmp and replace them with rpm_list
            if len(df_map):
                df_fmp = df_fmp.update(df_map, on='row_idx')

        print(f"Dropping long dated earnings with no reporting_periods: {fmp_drop_list}...")
        df_fmp = df_fmp.filter(~pl.col('row_idx').is_in(fmp_drop_list))

        print(f"Set remaining unmatched earnings reports to future reporting_periods...")
        tmp_date_now = datetime.now(timezone.utc)
        tmp_date_fut = datetime.fromisoformat(self._tmp_date_fut).replace(tzinfo=timezone.utc).date()
        df_fmp = df_fmp.with_columns(
        ...
        uq_cols = ['asset_id', 'filing_date']
        df_rp_map = (df_rp_map.filter(pl.col('filing_date') == tmp_date_fut)
                              .rename({'id': 'reporting_period_id'})
                              .drop('row_idx')
        )
        df_fmp = (df_fmp.update(df_rp_map, on=uq_cols)
                        .drop(['asset_id', 'filing_date', 'row_idx'])
        )
        print(f"  Null reporting_period_id(s) found: {
    ...
    def update_table_eod_price(self):
        ...
        df_fmp = self.fmp_client.fetch_fmp_list_df(self.fmp_client.historical_full,
                                                   'symbol',
                                                   symbol=sym_list,
